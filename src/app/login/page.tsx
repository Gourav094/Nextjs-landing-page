    ...
    return (
        <div className = "flex flex-col gap-4 items-center justify-center min-h-screen min-w-sceen bg-violet-100 text-gray-800">
            ...
            <div className="flex flex-col  gap-2 items-start w-1/4">
                ...
                <input placeholder="username" type="text" value={user.username} onChange={(e) => setUser({...user,username:e.target.value})} id="username" className=" outline-none rounded-lg bg-gray-300 text-gray-800 px-3 py-2 w-full" />
            </div>
            <div className="flex flex-col  gap-2 items-start w-1/4">
            ...
            <Link href="/signup">Don&apos;t have account? Login here</Link>
            <button className="text-slate-200 border py-2 px-4 rounded-lg bg-gray-900" onClick={() => console.log(user)}>Login</button>
        </div>
    )
}